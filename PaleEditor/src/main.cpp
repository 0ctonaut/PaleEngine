...
//// camera
PaleRdr::CCamera camera(glm::vec3(0.0f, 0.0f, 3.0f));
...
int main(int, char**)
{
    ...
    PaleRdr::CWindowOpenGL Window(title.c_str());
    ...
    PaleRdr::CFrameBufferOpenGL frameBuffer(1920, 1080);

    PaleUI::CPanelManager PanelManager;
...
void processInput(GLFWwindow* window)
{
    ...
    if (bMouseRightButtonPress)
    {
        ...
        if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
            camera.ProcessKeyboard(PaleRdr::ECameraMove::FORWARD, deltaTime);
        if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
            camera.ProcessKeyboard(PaleRdr::ECameraMove::BACKWARD, deltaTime);
        if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
            camera.ProcessKeyboard(PaleRdr::ECameraMove::LEFT, deltaTime);
        if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
            camera.ProcessKeyboard(PaleRdr::ECameraMove::RIGHT, deltaTime);
        if (glfwGetKey(window, GLFW_KEY_E) == GLFW_PRESS)
            camera.ProcessKeyboard(PaleRdr::ECameraMove::UP, deltaTime);
        if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS)
            camera.ProcessKeyboard(PaleRdr::ECameraMove::DOWN, deltaTime);
    }
}
